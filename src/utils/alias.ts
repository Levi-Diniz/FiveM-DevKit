...
const CURSOR_KIT_DIR = ".fivem-devkit";
...
# Generated by fivem-devkit for instance: ${instanceName}
...
if [ ! -d "\$APP_PATH" ]; then
  ...
  echo "The instance may have been removed. Run 'fivem-devkit instance -l' to see available instances." >&2
  exit 1
fi
...
function generateShellFunction(
  ...
  return `
# fivem-devkit alias: ${aliasName} -> ${instanceName}
${aliasName}() {
  local DIR="\${1:-.}"
  if [[ "\$DIR" != /* ]]; then
    DIR="\$(cd "\$DIR" 2>/dev/null && pwd)"
...
# end fivem-devkit alias: ${aliasName}
`;
}
...
export function createShellAlias(
  ...
  if (existingContent.includes(`# fivem-devkit alias: ${aliasName}`)) {
...
export function removeShellAlias(
  ...
  const startMarker = `# fivem-devkit alias: ${aliasName}`;
  const endMarker = `# end fivem-devkit alias: ${aliasName}`;
