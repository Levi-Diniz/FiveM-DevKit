...
description: "Learn how to use FiveM DevKit's instance command to run multiple Cursor IDE instances simultaneously on macOS, enabling parallel development workflows with different accounts."
date: "2025-12-17"
author: "FiveM DevKit Team"
tags:
  ["cursor-ide", "multi-account", "productivity", "macos", "developer-tools"]
featured: true
---

Managing multiple Cursor IDE accounts is a common challenge for developers who work across different organizations, freelance projects, or need to separate personal and professional workflows. The **FiveM DevKit `instance` command** provides a powerful solution that enables you to run multiple Cursor instances simultaneously on macOS.
...
## Introducing FiveM DevKit Instance

FiveM DevKit's `instance` command creates isolated copies of Cursor IDE, each with its own identity and data directory. This means you can:
...
First, install FiveM DevKit globally:
...
```bash
fivem-devkit instance -a create -n "Cursor Work"
```
...
fivem-devkit instance -a create -n "Cursor Work" -A cursor-work

# Or add an alias to an existing instance
fivem-devkit instance -a alias -n "Cursor Work" -A cursor-work
```
...
```bash
fivem-devkit instance -a create -n "Cursor Work" -A cursor-work --aliasLocation shell-config
```
...
```bash
fivem-devkit instance -a reinstall -n "Cursor Work"
```
...
```bash
fivem-devkit instance -a remove -n "Cursor Work"
```
...
fivem-devkit instance -a create -n "Cursor Enterprise" -A cursor-enterprise

# Step 2: Create personal instance
fivem-devkit instance -a create -n "Cursor Personal" -A cursor-personal
...
fivem-devkit instance -a reinstall -n "Cursor Enterprise"
fivem-devkit instance -a reinstall -n "Cursor Personal"
```
...
```bash
# Good
fivem-devkit instance -a create -n "Cursor Client ABC"
fivem-devkit instance -a create -n "Cursor OpenSource"

# Avoid
fivem-devkit instance -a create -n "Cursor 2"
```
...
```bash
fivem-devkit instance -a reinstall -n "Your Instance Name"
```
...
```bash
sudo fivem-devkit instance -a alias -n "Instance" -A alias-name --aliasLocation usr-local-bin
```

## Conclusion

FiveM DevKit's `instance` command transforms how you work with multiple Cursor accounts. By creating isolated instances with custom shell aliases, you can seamlessly switch between different contexts without logging in and out.
...
> **Ready to get started?** Install FiveM DevKit today and create your first instance:
>
> ```bash
> npm install -g fivem-devkit-cli
> fivem-devkit instance -a create -n "My First Instance"
> ```

---

_FiveM DevKit is an open-source project that helps developers supercharge their AI IDE experience. Check out our [GitHub repository](https://github.com/duongductrong/fivem-devkit) for more features and updates._
